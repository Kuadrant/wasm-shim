...
mod tests {
    ...
    fn extended_string_fn() {
        ...
        assert_eq!(
            e.eval(&req_ctx, &mut cel_ctx),
            Ok(AttributeState::Available(2.into()))
        );
        let e =
        ...
        assert_eq!(
            e.eval(&req_ctx, &mut cel_ctx),
            Ok(AttributeState::Available("".into()))
        );
        let e = Expression::new("['hello', 'mellow'].join(' ')").expect("This must be valid CEL");
        assert_eq!(
            e.eval(&req_ctx, &mut cel_ctx),
        ...
        assert_eq!(
            e.eval(&req_ctx, &mut cel_ctx),
            Ok(AttributeState::Available("trim".into()))
        );
