...
impl AuthService {
        ...
        T: AttributeResolver,
    {
        ...
        attr.set_request(build_request()?);
        attr.set_destination(auth::build_peer(
        ...
        attr.set_source(auth::build_peer(
...
fn build_request() -> Result<AttributeContext_Request, PropertyError> {
    ...
    http.set_host(
        get_attribute::<String>(&"request.host".into())?.ok_or(PropertyError::Get(
    ...
    http.set_path(
        get_attribute::<String>(&"request.path".into())?.ok_or(PropertyError::Get(
