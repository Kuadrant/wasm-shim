...
#[test]
#[serial]
fn it_processes_sse_usage_event() {
    ...
    let sse_event = b"data: {\"usage\":{\"total_tokens\":11}}\n\n";
    module
        .call_proxy_on_response_body(http_context, sse_event.len() as i32, false)
        .expect_log(
            Some(LogLevel::Debug),
            Some(
                format!(
        ...
        .expect_log(
            Some(LogLevel::Debug),
            Some("#2 Initializing SSE buffer for streaming response"),
        )
        .expect_log(
            Some(LogLevel::Debug),
            Some(
                format!(
                    "#2 handle_sse_stream: body_size: {}, end_of_stream: false",
                    sse_event.len()
                )
                .as_str(),
            ),
        ...
        .returning(Some(sse_event))
        .expect_log(
            Some(LogLevel::Debug),
            Some(
                "#2 handle_sse_stream: processing chunk: data: {\"usage\":{\"total_tokens\":11}}\n\n",
            ),
        )
        .expect_log(
            Some(LogLevel::Debug),
            Some(
                "#2 handle_sse_stream: processing event data: {\"usage\":{\"total_tokens\":11}}",
            ),
        )
        .expect_log(
            Some(LogLevel::Debug),
            Some(
                "#2 handle_sse_stream: found usage data: {\"usage\":{\"total_tokens\":11}}",
            ),
        )
